package kr.mooner510.konopuro.domain.game.data.card.dto

import kr.mooner510.konopuro.domain.game._preset.PassiveType
import kr.mooner510.konopuro.domain.game._preset.TierType
import kr.mooner510.konopuro.domain.game.data.card.entity.StudentCardData
import kr.mooner510.konopuro.domain.game.data.card.entity.PlayerStudentCard
import kr.mooner510.konopuro.domain.game.data.card.types.CardType
import kr.mooner510.konopuro.domain.game.data.global.types.MajorType
import java.util.*

data class GameCard(
    val playerCardId: UUID,
    val groups: Set<MajorType>,
    val type: CardType,
    val tiers: EnumSet<TierType>,
    val passives: EnumSet<PassiveType>
) {
    companion object {
        fun new(playerStudentCard: PlayerStudentCard, studentCardData: StudentCardData): GameCard = GameCard(
            playerStudentCard.id,
            studentCardData.groupSet(),
            studentCardData.type,
            EnumSet.copyOf(
                listOfNotNull(
                    studentCardData.startTier,
                    playerStudentCard.tierSecond,
                    playerStudentCard.tierThird,
                    playerStudentCard.tierForth
                )
            ),
            EnumSet.copyOf(
                listOfNotNull(
                    studentCardData.passiveFirst,
                    studentCardData.passiveSecond,
                    studentCardData.passiveThird,
                )
            )
        )
    }
}
